...
CREATE PROCEDURE dbo.restoredatabase 
		@backuplocation VARCHAR(MAX), @prefix varchar(255), @dbname VARCHAR(255)
AS 
BEGIN 
	...
	CREATE TABLE #tblBackupFiles
		...
		SELECT @sql = @sql + char(13) + ' MOVE ''' + LogicalName + ''' TO ''' + @restorelocation + LogicalName + @prefix + '.' + RIGHT(PhysicalName,CHARINDEX('\',PhysicalName)) + ''','
		FROM #tblBackupFiles
		WHERE IsPresent = 1
